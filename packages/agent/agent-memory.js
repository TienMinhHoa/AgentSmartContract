...
async function appendToJsonFile(filePath, newData) {
	...
	try {
		await fs.writeFile(
			filePath,
			JSON.stringify({ Personality: character, History: oldData }, null, 2),
		);
	} catch (error) {
...
export async function getHistoryChat(chat_id = '') {
	...
	try {
		...
		await addHitoryChat(chat_id, {
			user: 'Here is the start of conversation between user and agent',
			system: 'Ok, I got it.',
		});
		return [
			new HumanMessage({ content: 'Here is the start of conversation between user and agent' }),
			new AIMessage({ content: 'Ok, I got it.' }),
		];
		// return []
	}
